...
class TestAccessTools:
    ...
    async def test_get_unifi_access_points(self):
        ...
        mock_client.get_access_points = AsyncMock(return_value=[{"id": "ap1", "name": "Access Point 1"}])
    ...
    async def test_get_unifi_access_users(self):
        ...
        mock_client = Mock(spec=AccessClient)
        mock_client.get_users = AsyncMock(return_value=[{"id": "user1", "name": "User 1"}])
    ...
    async def test_get_unifi_access_logs(self):
        ...
        mock_client = Mock(spec=AccessClient)
        mock_client.get_door_access_logs = AsyncMock(return_value=[{"id": "log1", "timestamp": "2023-01-01T00:00:00Z"}])
