...
class TestNetworkTools:
    ...
    async def test_get_unifi_sites(self):
        ...
        mock_client.get_sites = AsyncMock(return_value=[{"name": "default", "desc": "Default Site"}])
    ...
    async def test_get_unifi_devices(self):
        ...
        mock_client = Mock(spec=NetworkClient)
        mock_client.get_devices = AsyncMock(return_value=[{"mac": "aa:bb:cc:dd:ee:ff", "type": "uap"}])
    ...
    async def test_get_unifi_clients(self):
        ...
        mock_client = Mock(spec=NetworkClient)
        mock_client.get_clients = AsyncMock(return_value=[{"mac": "aa:bb:cc:dd:ee:ff", "hostname": "test-client"}])
    ...
    async def test_get_unifi_wlans(self):
        """Test getting WLANs."""
        mock_client = Mock(spec=NetworkClient)
        mock_client.get_wlans = AsyncMock(return_value=[{"name": "MyWiFi", "enabled": True}])
    ...
    async def test_restart_unifi_device(self):
        ...
        result = await restart_unifi_device(mock_client, "aa:bb:cc:dd:ee:ff", "test_site")
        
        mock_client.restart_device.assert_called_once_with("aa:bb:cc:dd:ee:ff", "test_site")
    ...
    async def test_get_unifi_statistics(self):
        ...
        mock_client = Mock(spec=NetworkClient)
        mock_client.get_statistics = AsyncMock(return_value={"rx_bytes": 1000, "tx_bytes": 2000})
